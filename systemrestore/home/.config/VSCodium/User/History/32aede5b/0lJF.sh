...
sudo cryptsetup luksHeaderBackup /dev/$STORAGE --header-backup-file /mnt/backup/file.img
}

function luks_convertkey() {
sudo cryptsetup luksConvertKey --key-slot 0 --pbkdf pbkdf2 --iter-time 500 /dev/$STORAGE
}
...
sed -ie 's/HOOKS=(\(.*\))/HOOKS=(\1 grub-btrfs-overlayfs)/' /etc/mkinitcpio.conf
sed -ie 's/HOOKS="\(.*\)"/HOOKS="\1 luks2"/' /etc/mkinitcpio.conf
